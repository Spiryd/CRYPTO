...
/// Generate a random BigInt in range [1, max-1] using rejection sampling (no division).
pub fn generate_random_bigint<const N: usize>(max: &BigInt<N>) -> BigInt<N> {
    ...
    loop {
        ...
        // Mask the top byte to keep within bit length
        if !bytes.is_empty() {
            bytes[0] &= top_mask;
        }
...
/// For odd modulus, routes through Montgomery for speed; falls back to slow path otherwise.
pub fn mod_pow<const N: usize>(
    ...
    // Fallback (rare): square-and-multiply using generic mod_mul
