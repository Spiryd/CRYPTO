...
fn main() {
    ...
    let tests: Vec<(ChallengeType, &str, fn(&ChallengeTestRunner) -> _)> = vec![
        ...
        (ChallengeType::Ecp, "ECP (EC over prime field)", |r| r.run_ecp_tests()),
        (ChallengeType::Ec2m, "EC2m (EC over binary field)", |r| r.run_ec2m_tests()),
        (ChallengeType::Ecpk, "ECPk (EC over extension field)", |r| r.run_ecpk_tests()),
    ];
    ...
    for (challenge_type, name, test_fn) in &tests {
        ...
        let sig_sym = if result.signature_success { "PASS" } else { "FAIL" };
        
        println!("  DH:  {} | Signature: {} | Time: {:.2}s", dh_sym, sig_sym, elapsed.as_secs_f64());
    ...
    for result in &results {
        let dh = if result.dh_success { total_pass += 1; "PASS" } else { total_fail += 1; "FAIL" };
        let sig = if result.signature_success { total_pass += 1; "PASS" } else { total_fail += 1; "FAIL" };
