...
impl<const N: usize> MontgomeryCtx<N> {
    ...
    pub fn mont_mul(&self, a: &BigInt<N>, b: &BigInt<N>) -> BigInt<N> {
        ...
        // t has N+1 limbs (little-endian). Using Vec for stable Rust.
        ...
        for i in 0..N {
            ...
            for j in 0..N {
                let uv = (t[j] as u128)
                    + (a.limbs()[j] as u128) * (bi as u128)
                    + carry;
                t[j] = uv as u64;
                carry = uv >> 64;
            }
            // add carry into t[N]
            t[N] = t[N].wrapping_add(carry as u64);
            ...
            // 4) t >>= 64 (drop limb 0)
            // stable + simple: shift down by one limb
            for j in 0..N {
                t[j] = t[j + 1];
            }
            t[N] = 0;
        ...
        let mut out = BigInt::<N>::from_limbs_internal(
            t[..N].try_into().expect("slice len"),
        );
